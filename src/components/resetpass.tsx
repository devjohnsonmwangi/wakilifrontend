...
interface PasswordValidityState { length: boolean; uppercase: boolean; number: boolean; specialChar: boolean; }

// ===================================================================
// --- FIX: Child components are defined OUTSIDE the parent component.
// This makes them "stable" and prevents them from being re-created
// on every keystroke, which solves the focus/keyboard issue.
// ===================================================================

const InvalidTokenView = () => (
    <div className="text-center animate-fade-in-up">
        <div className="flex justify-center mb-6"><IconBrokenLink /></div>
        ...
        <Link to="/forgot-password" className="inline-flex items-center justify-center w-full rounded-xl py-3 px-4 text-sm font-semibold transition-all duration-300 bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-500/20 hover:-translate-y-0.5">
            Request a New Link
        </Link>
    </div>
);

const SuccessView = ({ message, onProceed }: { message: string, onProceed: () => void }) => (
    <div className="text-center animate-fade-in-up">
        <div className="flex justify-center mb-6"><AnimatedCheckmark /></div>
        <h2 className="text-2xl font-bold text-slate-800">Password Updated!</h2>
        <p className="text-slate-600 mt-2 mb-8">{message}</p>
        <button onClick={onProceed} className="group relative overflow-hidden flex w-full justify-center rounded-xl border border-transparent bg-gradient-to-r from-green-500 to-emerald-600 py-3.5 px-4 text-sm font-semibold text-white shadow-lg shadow-green-500/30 transition-all duration-300 ease-in-out hover:shadow-xl hover:-translate-y-0.5">
            Proceed to Login →
        </button>
    </div>
);
...
interface PasswordFormViewProps {
    ...
    newPassword: { value: string; setter: (v: string) => void };
    confirmPassword: { value: string; setter: (v: string) => void };
    showPassword: { value: boolean; setter: (v: boolean) => void };
    passwordValidity: PasswordValidityState;
    isLoading: boolean;
    clientError: string | null;
    apiErrorMessage: string | null;
}

const PasswordFormView: React.FC<PasswordFormViewProps> = ({
    mode, handleSubmit, newPassword, confirmPassword, showPassword, passwordValidity, isLoading, clientError, apiErrorMessage
}) => (
    <div className="animate-fade-in-up">
        <h2 className="text-3xl font-bold text-slate-900 text-center mb-2">{mode === 'reset' ? 'Set a New Password' : 'Change Your Password'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            <div>
                ...
                <div className="relative">
                    ...
                    <input type={showPassword.value ? 'text' : 'password'} id="newPassword" value={newPassword.value} onChange={(e) => newPassword.setter(e.target.value)} required disabled={isLoading} placeholder="Enter new password" className="block w-full rounded-xl border border-slate-300 bg-white/80 py-3 pl-11 pr-12 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                    <button type="button" onClick={() => showPassword.setter(!showPassword.value)} className="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-500 hover:text-indigo-600">{showPassword.value ? <IconEyeSlash /> : <IconEye />}</button>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="relative">
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5"><IconLock /></div>
                    <input type={showPassword.value ? 'text' : 'password'} id="confirmPassword" value={confirmPassword.value} onChange={(e) => confirmPassword.setter(e.target.value)} required disabled={isLoading} placeholder="Confirm new password" className="block w-full rounded-xl border border-slate-300 bg-white/80 py-3 pl-11 pr-12 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                </div>
            </div>
            {(clientError || apiErrorMessage) && (<div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{clientError || apiErrorMessage}</div>)}
            <div>
                <button type="submit" disabled={isLoading} className="group relative overflow-hidden flex w-full justify-center rounded-xl border border-transparent bg-gradient-to-r from-indigo-600 to-purple-600 py-3.5 px-4 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 disabled:opacity-50 disabled:shadow-none disabled:hover:translate-y-0 active:scale-[0.98]">
...
const ResetPasswordForm: React.FC<ResetPasswordFormProps> = ({ token, mode }) => {
    ...
    const [passwordValidity, setPasswordValidity] = useState<PasswordValidityState>({ length: false, uppercase: false, number: false, specialChar: false });
    ...
    const renderContent = () => {
        ...
        if (isSuccess) {
            return <SuccessView 
                message={data?.msg || "Your password has been changed successfully. You can now log in with your new credentials."} 
        ...
        return <PasswordFormView
            ...
            newPassword={{ value: newPassword, setter: setNewPassword }}
            confirmPassword={{ value: confirmPassword, setter: setConfirmPassword }}
            showPassword={{ value: showPassword, setter: setShowPassword }}
