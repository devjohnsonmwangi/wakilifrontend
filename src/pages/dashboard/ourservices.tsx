...
import {
    ...
    // These are now all used in the data objects below
    UsersRound, Puzzle, Target, Settings, MessageSquare, CheckCircle 
} from 'lucide-react';

// --- INTERFACES AND DATA (FULLY DEFINED TO FIX ERRORS) ---
...
const initialServicesData: Omit<ServiceItem, 'detailedDescription' | 'whoIsThisFor' | 'processSteps' | 'expertBlurb' | 'pricingModel' | 'faqs'>[] = [
    { id: 'svc001', slug: 'legal-consultation', icon: Clipboard, title: 'Legal Consultation', description: 'Book a consultation with our expert legal advisors for personalized advice for any matter.', category: 'Advisory', tags: ['consultation', 'advice', 'expert'], isPopular: true, status: 'Active', lastUsed: '5 days ago' },
    { id: 'svc002', slug: 'contract-drafting-review', icon: ShieldCheck, title: 'Contract Drafting & Review', description: 'Ensure your contracts are legally sound and protect your interests effectively. Covers all contract types.', category: 'Documentation', tags: ['contracts', 'review', 'legal docs', 'drafting'], status: 'Active' },
    { id: 'svc003', slug: 'corporate-law-services', icon: Briefcase, title: 'Corporate Law & Business Setup', description: 'Comprehensive services including company formation, M&A, compliance, and shareholder agreements.', category: 'Business', tags: ['corporate', 'formation', 'mergers', 'compliance'], isNew: true, status: 'Active' },
    { id: 'svc004', slug: 'litigation-dispute-resolution', icon: Scale, title: 'Litigation & Dispute Resolution', description: 'Expert representation in mediation, arbitration, and complex court proceedings. We fight for you.', category: 'Disputes', tags: ['litigation', 'court', 'arbitration', 'mediation'], status: 'Active', lastUsed: '1 month ago' },
    { id: 'svc005', slug: 'employment-law-support', icon: Users, title: 'Employment & Labor Law', description: 'Guidance on employment contracts, workplace disputes, HR policies, and labor compliance.', category: 'Business', tags: ['employment', 'hr', 'labor', 'workplace'], isPopular: true, status: 'Active' },
    { id: 'svc006', slug: 'intellectual-property-protection', icon: Truck, title: 'Intellectual Property Protection', description: 'Secure your trademarks, patents, copyrights, and trade secrets with our expert IP team.', category: 'Protection', tags: ['ip', 'trademark', 'patent', 'copyright', 'trade secret'], status: 'Active' },
    { id: 'svc007', slug: 'oaths-commissioning-notary', icon: FileText, title: 'Oaths & Notary Services', description: 'Official witnessing for affidavits, statutory declarations, and notarization of legal documents.', category: 'Official Services', tags: ['notary', 'affidavit', 'declaration', 'commissioner'], isNew: true, status: 'Active' },
    { id: 'svc008', slug: 'family-law-divorce', icon: Users, title: 'Family Law & Divorce', description: 'Compassionate support for divorce, child custody, spousal support, adoption, and related matters.', category: 'Personal', tags: ['family', 'divorce', 'custody', 'adoption', 'support'], status: 'Active', lastUsed: 'Not used yet' },
    { id: 'svc009', slug: 'real-estate-law', icon: Landmark, title: 'Real Estate Law', description: 'Navigating property transactions, leasing, zoning, and land use regulations. For buyers and sellers.', category: 'Property', tags: ['real estate', 'property', 'conveyancing', 'leasing', 'zoning'], isPopular: true, status: 'Active' },
    { id: 'svc010', slug: 'financial-legal-services', icon: DollarSign, title: 'Financial & Banking Law', description: 'Advice on financial regulations, loan agreements, investment compliance, and fintech law.', category: 'Finance', tags: ['finance', 'banking', 'investment', 'fintech', 'regulation'], status: 'Active' },
    { id: 'svc011', slug: 'startup-legal-kickstarter', icon: Car, title: 'Startup Legal Kickstarter', description: 'Essential legal services for new businesses, from incorporation to initial funding rounds.', category: 'Business', tags: ['startup', 'new business', 'incorporation', 'funding'], isNew: true, status: 'Active' },
    { id: 'svc012', slug: 'estate-planning-wills', icon: GanttChartSquare, title: 'Estate Planning & Wills', description: 'Secure your legacy with comprehensive estate planning, will drafting, and trust formation.', category: 'Personal', tags: ['estate planning', 'wills', 'trusts', 'probate'], status: 'Active' },
    { id: 'svc013', slug: 'immigration-law-services', icon: Users, title: 'Immigration Law', description: 'Assistance with visa applications, residency permits, citizenship, and immigration appeals.', category: 'Immigration', tags: ['immigration', 'visa', 'residency', 'citizenship'], isPopular: true, status: 'Active', lastUsed: '2 weeks ago' },
    { id: 'svc014', slug: 'dispute-mediation-service', icon: Wrench, title: 'Alternative Dispute Resolution (ADR)', description: 'Facilitating resolutions outside of court through expert mediation and negotiation services.', category: 'Disputes', tags: ['adr', 'mediation', 'negotiation', 'settlement'], status: 'Active' },
    { id: 'svc015', slug: 'archived-legacy-service', icon: Clipboard, title: 'Legacy Data Archival (Old)', description: 'Access to archived records from our previous data management system. Read-only access.', category: 'Archived', tags: ['legacy', 'archive', 'old records'], status: 'Active', lastUsed: '1 year ago' }
];
...
const specificDetailedContentMap: Record<string, Partial<ServiceItem>> = {
    'svc001': {
        ...
        whoIsThisFor: ["Individuals with personal legal questions", "Small business owners needing quick advice", "Anyone facing a potential legal dispute"],
        processSteps: [
        ...
        faqs: [{question: "Is this confidential?", answer: "Yes, all consultations are protected by attorney-client privilege."}]
    },
    'svc002': {
        ...
        whoIsThisFor: ["Businesses of all sizes", "Landlords and tenants", "Freelancers and contractors"],
        processSteps: [
        ...
        faqs: [{question: "How long does a review take?", answer: "Standard reviews are typically completed within 2-3 business days."}]
    },
    'svc003': {
        ...
        whoIsThisFor: ["Entrepreneurs starting a new venture", "Established corporations", "Investors and shareholders"],
        processSteps: [
            ...
            { title: "Execution & Filing", description: "Handle all necessary documentation and governmental filings.", icon: FileText },
            { title: "Ongoing Support", description: "Provide continued advice on corporate governance and compliance.", icon: UsersRound }
        ],
        ...
        faqs: [{question: "What type of company should I form?", answer: "We can advise on the best structure (e.g., LLC, C-Corp) based on your specific goals and circumstances."}]
    },
};
...
const allDashboardServices: ServiceItem[] = initialServicesData.map(baseService => {
    ...
    if (specificDetails) { return { ...baseService, ...specificDetails, detailedDescription }; }
    return { ...baseService, ...genericDetailedContentDefaults, detailedDescription, };
});
...
const CustomDropdown = <T extends string>({ label, options, selectedValue, onSelect }: CustomDropdownProps<T>) => {
    ...
    return (
        <div className="md:col-span-1" ref={dropdownRef}>
            <label htmlFor={label} className="block text-sm font-extrabold text-slate-700 dark:text-slate-300 mb-1">
                {label}
            </label>
            <div className="relative">
                <button
                    ...
                    // MODIFIED: Added font-extrabold
                    className="w-full pl-3 pr-10 py-2.5 text-left border border-slate-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none bg-transparent text-slate-700 dark:text-slate-200 transition-colors flex items-center justify-between font-extrabold"
                >
                ...
                {isOpen && (
                    <ul className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 shadow-lg rounded-md ring-1 ring-black ring-opacity-5 py-1 max-h-60 overflow-y-auto">
                        {options.map((option) => (
                            <li
                                key={option.value}
                                onClick={() => {
                                ...
                                className={`px-4 py-2 text-sm cursor-pointer ${
                                    selectedValue === option.value
                                        ...
                                        : 'text-slate-700 dark:text-slate-200 hover:bg-purple-100 dark:hover:bg-slate-700'
                                }`}
                            >
                                {option.label}
                            </li>
...
const OurServices: React.FC = () => {
    ...
    const processedServices = useMemo(() => {
        ...
        switch (sortBy) {
            case 'title-asc': services.sort((a, b) => a.title.localeCompare(b.title)); break;
            case 'title-desc': services.sort((a, b) => b.title.localeCompare(a.title)); break;
            case 'popularity': services.sort((a, b) => (b.isPopular ? 1 : 0) - (a.isPopular ? 1 : 0) || a.title.localeCompare(b.title)); break;
            case 'newest': services.sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0) || a.title.localeCompare(b.title)); break;
            case 'category': services.sort((a, b) => a.category.localeCompare(b.category) || a.title.localeCompare(b.title)); break;
        }
        return services;
    ...
    // FIXED: Implemented function to resolve "unused parameter" error
    ...
    return (
        <div className="flex-1 p-4 sm:p-6 md:p-8 bg-gray-100 dark:bg-slate-900 min-h-screen transition-colors duration-300">
            <header className="mb-8 md:mb-12 p-6 sm:p-8 md:p-10 bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-700 dark:from-purple-700 dark:via-indigo-700 dark:to-blue-800 rounded-xl shadow-xl text-center">
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight mb-4">Explore Our Premier Legal Services</h1>
                <p className="text-md sm:text-lg text-purple-100 dark:text-purple-200 max-w-3xl mx-auto">Discover tailored solutions for all your legal needs. Expertise and dedication, at your service.</p>
            </header>

            <div className="mb-6 md:mb-8 p-4 bg-white dark:bg-slate-800 rounded-lg shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div className="md:col-span-1">
                        <label htmlFor="search-service" className="block text-sm font-extrabold text-slate-700 dark:text-slate-300 mb-1">Search Services</label>
                        <div className="relative">
                            <Search className="absolute inset-y-0 left-0 pl-3 h-full flex items-center pointer-events-none text-slate-400 dark:text-slate-500" />
                            <input
                                id="search-service" type="text" placeholder="E.g., 'contract', 'IP'"
                                value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                                // MODIFIED: Added font-extrabold
                                className="w-full pl-10 pr-10 py-2.5 border border-slate-300 dark:border-slate-600 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none bg-transparent text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 transition-colors font-extrabold"
                            />
                            {searchTerm && (
                                <button
                                    onClick={() => setSearchTerm('')}
                                    className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-purple-600 dark:text-slate-500 dark:hover:text-purple-400"
                        ...
                        >
                            <div className="p-3 sm:p-4 border-b border-slate-200 dark:border-slate-700/50">
                                <div className="flex justify-between items-center">
                                    <span className="text-xs font-semibold uppercase tracking-wider text-purple-600 dark:text-purple-400">{service.category}</span>
                                    <div className="flex space-x-2">
                                        {service.isNew && <span title="New Service" className="flex items-center text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-500/30 dark:text-green-300"><Zap size={12} className="mr-1" /> New</span>}
                                        {service.isPopular && <span title="Popular Service" className="flex items-center text-xs px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-500/30 dark:text-yellow-300"><Star size={12} className="mr-1" /> Popular</span>}
                                    </div>
                                </div>
                            </div>
                            <div className="p-4 sm:p-6 flex-grow">
                                <div className="flex items-start mb-3">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-semibold text-slate-800 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-300 transition-colors duration-300 mb-1">{service.title}</h2>
                                        {service.status && <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${getStatusColor(service.status)}`}>{service.status}</span>}
                                    </div>
                                </div>
                                <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 leading-relaxed line-clamp-3">{service.description}</p>
                                {service.lastUsed && <p className="text-xs text-slate-500 dark:text-slate-500 mt-2"><Info size={12} className="inline mr-1" />Last used: {service.lastUsed}</p>}
                            </div>
