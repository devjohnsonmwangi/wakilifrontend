...
const DocumentList: React.FC = () => {
    ...
    const handleUpdate = async (document_id: number) => {
        const document = documents?.find((doc: Document) => doc.document_id === document_id);
        if (!document) return;
        setSelectedDocumentId(document_id);
    ...
    // First, get all documents that are valid and have a non-null case_id
    const documentsWithCaseId: Document[] = documents
        ? (documents as Document[]).filter(doc => {
            // Ensure the document object itself exists and case_id is not null or undefined
            return doc && doc.case_id != null;
        })
        : [];

    // Prepare the search term (trimmed and lowercased)
    const lowerTrimmedSearchTerm = searchTerm.trim().toLowerCase();

    // Filter documentsWithCaseId based on the search term
    const filteredDocuments: Document[] = lowerTrimmedSearchTerm
        ? documentsWithCaseId.filter(document => {
            // At this point, 'document' is guaranteed to be valid and have a case_id
            const docName = (document.document_name || '').toLowerCase();
            const caseIdString = document.case_id!.toString(); // case_id is not null, convert to string for searching

            // Apply search logic:
            // - Document name (case-insensitive) contains the search term
            // - OR Case ID (as string, compared with lowercased search term) contains the search term
            return docName.includes(lowerTrimmedSearchTerm) ||
                   caseIdString.includes(lowerTrimmedSearchTerm);
          })
        : documentsWithCaseId; // If no search term (or only whitespace), show all documents that have a case_id
    ...
    return (
        <div className="w-full min-h-screen p-4 sm:p-6 bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300">
                            ...
                            {filteredDocuments.length > 0 ? filteredDocuments.map(doc => (
                                <tr key={doc.document_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150 border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {/* case_id is guaranteed to be non-null here due to filtering */}
                                ...
                                <tr>
                                    <td colSpan={tableHeaders.length} className="text-center py-10 text-slate-500 dark:text-slate-400">
                                        No documents found{searchTerm && " matching your search"}{!searchTerm && " with a Case ID"}.
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Deletion" size="sm">
                <DeleteCaseForm
                    caseItem={documents?.find((doc: Document) => doc.document_id === selectedDocumentId) || null}
