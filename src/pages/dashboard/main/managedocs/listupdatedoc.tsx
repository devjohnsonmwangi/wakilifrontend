...
const PencilSquareIcon = ({ className = "w-4 h-4" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
const IdentificationIcon = ({ className = "w-5 h-5" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
    </svg>
);
const CalendarDaysIcon = ({ className = "w-5 h-5" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V10.5m-9-3.75h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" />
    </svg>
);
const FolderIcon = ({ className = "w-5 h-5" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z" />
    </svg>
);
const UsersIcon = ({ className = "w-5 h-5" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.125-2.257 9.337 9.337 0 0 0-4.125-2.257m0 4.514v-4.514M15 12a3.75 3.75 0 0 1-7.5 0 3.75 3.75 0 0 1 7.5 0Zm-7.5 0v-1.125c0-1.02.83-1.875 1.875-1.875h3.75c1.045 0 1.875.855 1.875 1.875v1.125m-7.5 0h7.5m-7.5 0a9.004 9.004 0 0 0-4.125 2.257 9.337 9.337 0 0 0-2.625.372m9.75 0a9.004 9.004 0 0 1-4.125 2.257 9.337 9.337 0 0 1-2.625.372" />
    </svg>
);
const AdjustmentsHorizontalIcon = ({ className = "w-5 h-5" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 0 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" />
    </svg>
);
const ExclamationTriangleIcon = ({ className = "w-6 h-6" }) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
        <path strokeLinecap="round" strokeLinejoin="round" d="12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
    </svg>
);

// Helper function to get structured error message
function getStructuredErrorMessage(error: unknown): string {
    ...
    if (typeof error === 'object' && error !== null) {
        ...
        if (errObj.data && typeof errObj.data === 'object' && typeof (errObj.data as { message?: unknown }).message === 'string') {
            return (errObj.data as { message: string }).message;
        }
        if (errObj.data && typeof errObj.data === 'object' && typeof (errObj.data as { detail?: unknown }).detail === 'string') {
            return (errObj.data as { detail: string }).detail;
        }
        if (errObj.data && typeof errObj.data === 'object' && typeof (errObj.data as { error?: unknown }).error === 'string') {
...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, title, size = 'md' }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 transition-opacity duration-300 ease-in-out" onClick={onClose} aria-modal="true" role="dialog">
            <div className={`relative bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 shadow-2xl rounded-lg transition-all duration-300 ease-in-out w-full ${sizeClasses[size]} flex flex-col overflow-hidden transform scale-95 opacity-0 animate-modalShow`} onClick={(e) => e.stopPropagation()}>
                {(title) && (
                    <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                        {title && <h2 className="text-xl font-semibold">{title}</h2>}
                        <button onClick={onClose} className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors" aria-label="Close modal">
...
const DocumentList: React.FC = () => {
    ...
    const tableHeaders = [
        { text: "Name", icon: DocumentTextIcon },
        { text: "Doc ID", icon: IdentificationIcon },
        { text: "Updated", icon: CalendarDaysIcon },
        { text: "Case Number", icon: FolderIcon },
        { text: "Parties", icon: UsersIcon },
        ...
        <div className="w-full min-h-screen p-4 sm:p-6 bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300">
                        ...
                        <thead className="bg-blue-600 dark:bg-blue-700 text-white dark:text-blue-50">
                            <tr>
                                {tableHeaders.map(header => (
                                    <th key={header.text} className="px-4 py-3 border-b-2 border-blue-700 dark:border-blue-800 text-left text-xs font-bold uppercase tracking-wider">
                                ...
                                <tr key={doc.document_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150 border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <p className="text-slate-900 dark:text-slate-100 whitespace-nowrap font-medium truncate" title={doc.document_name}>{doc.document_name}</p>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <div className="flex items-center space-x-2">
                                    ...
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <p className="text-slate-600 dark:text-slate-300 whitespace-nowrap">{doc.case?.case_number || 'N/A'}</p>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <div className="flex items-center space-x-2">
                                            <UsersIcon className="w-4 h-4 text-slate-400 dark:text-slate-500 flex-shrink-0" />
                                            <p className="text-slate-600 dark:text-slate-300 whitespace-nowrap truncate max-w-xs" title={doc.case?.parties}>{doc.case?.parties || 'N/A'}</p>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 text-sm font-semibold">
                                        <div className="flex items-center space-x-2">
                                            <button onClick={() => handleDownload(doc.document_url)} title="Open/Download Document" className={`${tableActionButtonTextClass} text-white bg-sky-500 hover:bg-sky-600 dark:bg-sky-600 dark:hover:bg-sky-700 focus:ring-sky-300 dark:focus:ring-sky-800`}>
                                                <ExternalLinkIcon className="w-3.5 h-3.5" /><span>Open</span>
                                            </button>
                                            <button onClick={() => handleUpdate(doc.document_id)} title="Update Document" className={`${tableActionButtonTextClass} text-white bg-amber-500 hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-700 focus:ring-amber-300 dark:focus:ring-amber-800`}>
                                                <PencilSquareIcon className="w-3.5 h-3.5" /><span>Update</span>
                                            </button>
                                            <button onClick={() => openDeleteModal(doc.document_id)} title="Delete Document" className={`${tableActionButtonTextClass} text-white bg-red-500 hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700 focus:ring-red-300 dark:focus:ring-red-800`}>
            ...
            <Modal isOpen={isUpdateModalOpen} onClose={closeUpdateModal} title="Update Document Details" size="md">
                <form onSubmit={handleSubmitUpdate} className="space-y-6">
                    <div>
                        <label htmlFor="update-name" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Document Name:</label>
                        <input type="text" id="update-name" className={inputBaseClass} value={name} onChange={(e) => setName(e.target.value)} required />
                    </div>
                    <div>
                        <label htmlFor="update-url" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Document URL (Reference):</label>
                        <input type="url" id="update-url" className={inputBaseClass} value={content} onChange={(e) => setContent(e.target.value)} placeholder="e.g., https://example.com/document.pdf" />
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Note: This typically updates a reference URL. To replace the file, use the create/upload functionality.</p>
                    </div>
                    <div className="flex justify-end space-x-3 pt-2">
                        <button type="button" onClick={closeUpdateModal} className={`${buttonBaseClass} bg-slate-200 hover:bg-slate-300 dark:bg-slate-600 dark:hover:bg-slate-500 text-slate-700 dark:text-slate-200 focus:ring-slate-300 dark:focus:ring-slate-700`}>Cancel</button>
                        <button className={`${primaryButtonClass}`} type="submit">Save Changes</button>
                    </div>
                </form>
            </Modal>
            <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Confirm Deletion" size="sm">
                <DeleteCaseForm caseItem={documents?.find((doc: CaseDocument) => doc.document_id === selectedDocumentId) || null} onClose={() => setIsDeleteModalOpen(false)} refetch={refetch} />
            </Modal>
        </div>
    );
};
