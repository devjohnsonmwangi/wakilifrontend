...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children, title, size = 'md' }) => {
    ...
    const sizeClasses = { sm: 'max-w-sm', md: 'max-w-md', lg: 'max-w-lg', xl: 'max-w-3xl', full: 'w-full h-full max-w-full max-h-full rounded-none sm:rounded-lg' };
    return (
            ...
            <div className={`relative bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 shadow-2xl rounded-lg transition-all duration-300 ease-in-out w-full ${sizeClasses[size]} flex flex-col overflow-hidden transform scale-95 opacity-0 animate-modalShow`} onClick={(e) => e.stopPropagation()}>
                {(title) && (
                    <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
                        ...
                        <button onClick={onClose} className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 transition-colors" aria-label="Close modal"><XMarkIcon className="w-6 h-6" /></button>
                    </div>
                )}
...
const LibDocumentList: React.FC = () => {
    ...
    // --- Get user authentication status and role from Redux state using your slice's selectors ---
    const isAuthenticated = useSelector(selectIsAuthenticated);
    const userRole = useSelector(selectUserRole); // This will be a string like 'admin', 'user', etc., or undefined

    // --- Define which roles have management permissions ---
    // Customize this based on your application's roles.
    // Example: 'admin' role can manage documents. You might have 'editor', 'manager', etc.
    const canManageDocuments = isAuthenticated && userRole === 'admin'; // Or: userRole === 'editor' || userRole === 'superuser'
    ...
    const handleUpdate = async (document_id: number) => {
        const documentToUpdate = documents?.find((doc) => doc.document_id === document_id);
        if (!documentToUpdate) return;
        setSelectedDocumentId(document_id);
        setName(documentToUpdate.document_name);
        setContent(documentToUpdate.document_url);
    ...
    const filteredDocuments = documents
        ? documents.filter(document => {
            ...
            if (document.case_id !== null && document.case_id !== undefined) {
                return false;
            }
        ...
        <div className="w-full min-h-screen p-4 sm:p-6 bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 transition-colors duration-300">
            ...
            <header className="mb-6 sm:mb-8">
                <div className="flex justify-between items-center mb-2">
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-500">
                        Our Library
                    </h1>
                ...
                <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400">
                    Access general legal documents, view insightful articles, and explore a wide range of shared content.
                </p>
            </header>

            <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
                {/* Only allow contributing if authenticated (or based on role if needed) */}
                {isAuthenticated && (
                    <button onClick={openCreateModal} className={`${primaryButtonClass} w-full sm:w-auto`}>
                        <PlusIcon className="w-5 h-5 mr-2" /> Contribute Document
                    </button>
                )}
                <div className="relative w-full sm:flex-grow">
                    ...
                    <input type="text" placeholder="Search by document name..." className={`${inputBaseClass} pl-10`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
            </div>
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-10 text-slate-600 dark:text-slate-400 text-center">
                    ...
                    <p className="text-lg font-medium">Loading Library Documents</p>
                ...
                <div className="my-8 p-6 sm:p-8 bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 dark:border-red-600 rounded-md shadow-md flex flex-col items-center text-center" role="alert">
                    ...
                    <p className="text-sm text-red-600 dark:text-red-400 mb-4 max-w-md">
                        We encountered an issue: <strong className="font-medium">{getStructuredErrorMessage(error)}</strong>
                    </p>
                    <button onClick={() => refetch()} className={`${primaryButtonClass} bg-red-500 hover:bg-red-600 dark:bg-red-600 dark:hover:bg-red-700 focus:ring-red-300 dark:focus:ring-red-700`}>
                        <PlusIcon className="w-4 h-4 mr-1.5 rotate-45 transform" /> Try Again
                    </button>
                </div>
            ) : (
                                ...
                                {tableHeaders.map(header => (
                                    <th key={header.text} className="px-4 py-3 border-b-2 border-blue-700 dark:border-blue-800 text-left text-xs font-semibold uppercase tracking-wider">
                                        <div className="flex items-center space-x-1.5 whitespace-nowrap">
                                            <header.icon className={`w-4 h-4 text-blue-200 dark:text-blue-300`} />
                            ...
                            {filteredDocuments.length > 0 ? filteredDocuments.map(doc => (
                                <tr key={doc.document_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150 border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            {/* --- Conditional rendering for Edit and Delete buttons based on user role --- */}
                                            {canManageDocuments && (
                                                <>
                                ...
                                <tr>
                                    <td colSpan={tableHeaders.length} className="text-center py-10 text-slate-500 dark:text-slate-400">
                                        No library documents found{searchTerm && " matching your search"}.
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            {/* Conditionally render create modal trigger */}
            {isAuthenticated && (
                 <Modal isOpen={isCreateModalOpen} onClose={closeCreateModal} title="Contribute to Our Library" size="xl">
                    <GeneralDocumentUpload onClose={closeCreateModal} />
                </Modal>
            )}

            {/* Conditionally render update modal trigger (implicitly by button, but modal itself can also be conditional) */}
            {canManageDocuments && (
                <Modal isOpen={isUpdateModalOpen} onClose={closeUpdateModal} title="Update Library Document" size="md">
                    <form onSubmit={handleSubmitUpdate} className="space-y-6">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Note: This updates the reference URL. To replace the actual file, use "Contribute Document".</p>
                        </div>
