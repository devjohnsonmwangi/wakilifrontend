                ...
                >
                    ...
                    >
                        <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                            ...
                            <button onClick={onClose} title="Close" aria-label="Close" className="p-2 rounded-full hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                <X className="h-6 w-6" />
                            </button>
                        </header>
...
const AllTicket = () => {
   ...
   // const [addLog] = logAPI.useCreateLogMutation();
    ...
    useEffect(() => { if (isError && allUserTickets && allUserTickets.length > 0 && !isLoading) showToast('âš ï¸ Could not refresh ticket data.', 'error', 5000); }, [isError, allUserTickets, isLoading, showToast]);
