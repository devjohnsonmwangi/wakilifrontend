...
// --- MODAL IMPORTS ---
import { CaseProgressModal } from './progress'; // Your existing progress modal
import { CaseDocumentsModal } from '../managedocs/singlecasedoc'; // <-- 1. IMPORT THE NEW MODAL

// --- ICON IMPORTS ---
import {
    FaTimes, FaTag, FaMoneyBill, FaBriefcase, FaHashtag, FaFileAlt,
    FaBalanceScale, FaUniversity, FaBuilding, FaUsers,
    FaCreditCard, FaDollarSign, FaHistory, FaFileArchive // <-- Added new icon for documents
} from "react-icons/fa";
...
const ViewCaseDetailsModal: React.FC<ViewCaseDetailsModalProps> = ({ isDarkMode, selectedCase, closeModal, currentUserRole }) => {

    // --- STATE MANAGEMENT FOR MODALS ---
    const [isProgressModalOpen, setIsProgressModalOpen] = useState(false);
    const [isDocumentsModalOpen, setIsDocumentsModalOpen] = useState(false); // <-- 2. STATE FOR DOCUMENTS MODAL
    ...
    const handleClose = () => {
        toast.dismiss(); // Dismiss any active toasts on close
        closeModal();
    };
    ...
    return (
        <>
            ...
            >
                ...
                >
                    ...
                    <div className="p-5 sm:p-6 overflow-y-auto flex-grow styled-scrollbar">
                        ...
                        {/* --- 3. NEW CASE DOCUMENTS SECTION --- */}
                        <div className="mt-6 pt-1">
                            <label className="flex items-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                                <FaFileArchive size={14} className="mr-2 text-indigo-600 dark:text-indigo-400 opacity-90" />
                                Case Documents
                            </label>
                            <div className="bg-slate-100 dark:bg-slate-700/60 border border-slate-200 dark:border-slate-600/70 p-4 rounded-lg shadow-sm flex items-center justify-between flex-wrap gap-2">
                                <p className="text-sm text-slate-700 dark:text-slate-300">
                                    View, upload, or manage all documents for this case.
                                </p>
                                <button
                                    onClick={() => setIsDocumentsModalOpen(true)} // <-- 4. BUTTON TO OPEN MODAL
                                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white text-sm font-medium rounded-lg shadow-md transition-colors"
                                >
                                    Manage Documents
                                </button>
            ...
            {/* Render the CaseProgressModal conditionally */}
            {isProgressModalOpen && (
                <CaseProgressModal
            ...
            {/* --- 5. RENDER THE CASE DOCUMENTS MODAL CONDITIONALLY --- */}
