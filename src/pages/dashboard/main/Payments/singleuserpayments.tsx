...
import {
    ...
    CreditCard, Activity, Eye, Download, UserX, LogOut, Settings, Loader2, Edit3, Sun, Moon, User, Info
} from 'lucide-react';
...
const UserPaymentPortal: React.FC = () => {
    ...
    const paymentModeButtonRef = useRef<HTMLButtonElement>(null);
    ...
    useEffect(() => {
        const handleClickOutsidePaymentDropdown = (event: MouseEvent) => {
            if (paymentModeDropdownRef.current && !paymentModeDropdownRef.current.contains(event.target as Node) && paymentModeButtonRef.current && !paymentModeButtonRef.current.contains(event.target as Node)) setIsPaymentModeSelectionOpen(false);
        ...
        return () => document.removeEventListener('mousedown', handleClickOutsidePaymentDropdown);
    }, []);
    ...
    const { data, isLoading: paymentsLoading, isError, error, refetch } = useFetchPaymentsQuery(undefined, { skip: !isAuthenticated || !currentUserId }) as FetchPaymentsQueryResult;
    ...
    const handleFilter = useCallback(() => {
        ...
        if (filters.transactionId) { const txId = filters.transactionId.trim().toLowerCase(); tempPayments = tempPayments.filter(p => (p.transaction_id && p.transaction_id.toLowerCase().includes(txId)) || (p.payment_gateway === 'mpesa' && (p.mpesa_message || '').toLowerCase().includes(txId))); }
    ...
    const handlePaymentModeSelect = (mode: PaymentMode) => {
        if (!isAuthenticated || !currentUserId) { toast.error("You must be logged in to initiate a payment."); return; }
    ...
    const handleClosePaymentModals = () => { setIsCashModalOpen(false); setIsMpesaModalOpen(false); setIsStripeModalOpen(false); setIsManualEntryModalOpen(false); if (isAuthenticated && currentUserId) refetch(); };
    
    const handleDownloadReceipt = async (payment: PaymentDataTypes) => {
        if (!payment.receipt_url || !payment.payment_id) { toast.error("Receipt URL not available."); return; }
    ...
    return (
        <div className="w-full p-2 sm:p-4 md:p-6 bg-slate-100 dark:bg-slate-900 min-h-screen font-sans">
            <header className="flex items-center justify-between mb-6 pb-4 border-b border-slate-200 dark:border-slate-700">
                <h1 className="text-xl sm:text-2xl font-semibold text-slate-800 dark:text-slate-100">{portalTitle}</h1>
            ...
            <div className="mb-6 text-right"><div ref={paymentModeDropdownRef} className="relative inline-block"><button ref={paymentModeButtonRef} type="button" className="inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-md text-sm" onClick={() => setIsPaymentModeSelectionOpen(p => !p)}><PlusCircle size={18} className="mr-2" /> Initiate New Payment <ChevronDown size={18} className={`ml-2 transform transition-transform ${isPaymentModeSelectionOpen ? 'rotate-180' : ''}`} /></button><AnimatePresence>{isPaymentModeSelectionOpen && ( <motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-xl bg-white dark:bg-slate-800 ring-1 ring-black dark:ring-slate-700 ring-opacity-5 z-20 py-1.5">{paymentModes.map(({ mode, label, icon }) => ( <button key={mode} onClick={() => handlePaymentModeSelect(mode)} className="flex items-center w-full text-left px-4 py-2.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">{icon} {label}</button> ))}<div className="border-t border-slate-200 dark:border-slate-700 my-1.5"></div><button onClick={() => setIsPaymentModeSelectionOpen(false)} className="flex items-center w-full text-left px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-700/30"><X size={16} className="mr-2.5 opacity-70" /> Cancel</button></motion.div> )}</AnimatePresence></div></div>
            
            <div className="mb-8 p-5 sm:p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5"><h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 flex items-center mb-3 sm:mb-0"> <ListFilter size={18} className="mr-2.5 text-indigo-500" /> Filter Payments </h3><button onClick={resetFilters} className="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg"><RotateCcw size={15} className="mr-1.5" /> Reset Filters</button></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div ref={statusDropdownRef} className="relative"><button type="button" onClick={() => setIsStatusDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`}><span className={`truncate font-semibold ${!filters.status ? 'text-slate-400 dark:text-slate-500 font-normal' : ''}`}>{filters.status ? filters.status.charAt(0).toUpperCase() + filters.status.slice(1) : 'All Statuses'}</span><div className="flex items-center">{filters.status && (<button type="button" onClick={(e) => { e.stopPropagation(); handleFilterChange('status', ''); }} title="Clear Status" className="p-1 mr-1 text-slate-400 hover:text-slate-600 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 ${isStatusDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isStatusDropdownOpen && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { handleFilterChange('status', ''); setIsStatusDropdownOpen(false); }}>All Statuses</button></div>{['pending', 'completed', 'paid', 'failed'].map(status => (<div key={status}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { handleFilterChange('status', status); setIsStatusDropdownOpen(false); }}>{status.charAt(0).toUpperCase() + status.slice(1)}</button></div>))}</motion.div>)}</AnimatePresence></div>
                    <div ref={gatewayDropdownRef} className="relative"><button type="button" onClick={() => setIsGatewayDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`}><span className={`truncate font-semibold ${!filters.gateway ? 'text-slate-400 dark:text-slate-500 font-normal' : ''}`}>{filters.gateway ? filters.gateway.charAt(0).toUpperCase() + filters.gateway.slice(1).replace('_', ' ') : 'All Gateways'}</span><div className="flex items-center">{filters.gateway && (<button type="button" onClick={(e) => { e.stopPropagation(); handleFilterChange('gateway', ''); }} title="Clear Gateway" className="p-1 mr-1 text-slate-400 hover:text-slate-600 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 ${isGatewayDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isGatewayDropdownOpen && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { handleFilterChange('gateway', ''); setIsGatewayDropdownOpen(false); }}>All Gateways</button></div>{['stripe', 'mpesa', 'cash', 'bank_transfer', 'other'].map(gw => (<div key={gw}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { handleFilterChange('gateway', gw); setIsGatewayDropdownOpen(false); }}>{gw.charAt(0).toUpperCase() + gw.slice(1).replace('_', ' ')}</button></div>))}</motion.div>)}</AnimatePresence></div>
                    <div className="relative"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" aria-hidden="true"/><input type="text" placeholder="Reference / Txn ID" value={filters.transactionId} onChange={(e) => handleFilterChange('transactionId', e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{filters.transactionId && (<button type="button" onClick={() => handleFilterChange('transactionId', '')} title="Clear Reference" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 rounded-full"><X size={16} /></button>)}</div>
                    <div className="relative"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" aria-hidden="true"/><input type="number" placeholder="Case ID" value={filters.caseId} onChange={(e) => handleFilterChange('caseId', e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{filters.caseId && (<button type="button" onClick={() => handleFilterChange('caseId', '')} title="Clear Case ID" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 rounded-full"><X size={16} /></button>)}</div>
                </div>
            </div>

            <div className="bg-white dark:bg-slate-800 shadow-xl rounded-xl overflow-hidden">
                <div className="overflow-x-auto"><table className="min-w-full"><thead className="bg-gradient-to-r from-blue-600 to-sky-600 text-white dark:from-blue-700 dark:to-sky-700"><tr>{[ { icon: Fingerprint, text: 'ID' }, { icon: Briefcase, text: 'Case ID' }, { icon: DollarSign, text: 'Amount' }, { icon: Info, text: 'Status' }, { icon: CreditCard, text: 'Gateway' }, { icon: CalendarDays, text: 'Date' }, { icon: Activity, text: 'Reference' }, { icon: Eye, text: 'Receipt' } ].map(h => <th key={h.text} className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider"><div className="flex items-center gap-2"><h.icon className="h-4 w-4" />{h.text}</div></th>)}</tr></thead><tbody className="text-slate-700 dark:text-slate-300 text-sm divide-y divide-slate-200 dark:divide-slate-700">{paymentsLoading && baseUserPayments.length === 0 ? (<tr><td colSpan={8} className="text-center py-12"><div className="flex justify-center items-center text-slate-500"><Loader2 size={24} className="animate-spin mr-3" />Loading...</div></td></tr>) : isError && baseUserPayments.length === 0 ? (<tr><td colSpan={8} className="text-center py-12"><div className="flex flex-col items-center text-red-500"><AlertTriangle size={40} className="mb-3" />Error loading payments.</div></td></tr>) : !paymentsLoading && baseUserPayments.length === 0 && isAuthenticated ? (<tr><td colSpan={8} className="text-center py-16 text-slate-500"><div className="flex flex-col items-center"><Search size={48} className="mb-4 text-slate-400" />You have no payment records.</div></td></tr>) : filteredPayments.length > 0 ? (filteredPayments.map((p) => { const statusInfo = getStatusDisplayInfo(p.payment_status); const currentIsDownloading = isDownloading === p.payment_id; return ( <tr key={p.payment_id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40"><td className="px-5 py-4"><div className="flex items-center"><Fingerprint size={15} className="mr-2 text-slate-400" /><span className="font-medium">{p.payment_id}</span></div></td><td className="px-5 py-4">{p.case_id || 'N/A'}</td><td className="px-5 py-4"><span className="font-semibold">KES {Number(p.payment_amount).toFixed(2)}</span></td><td className="px-5 py-4"><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${statusInfo.badgeClass} ${statusInfo.textClass}`}>{statusInfo.icon}{statusInfo.text}</span></td><td className="px-5 py-4 capitalize">{p.payment_gateway?.replace('_', ' ')}</td><td className="px-5 py-4">{formatDate(p.payment_date)}</td><td className="px-5 py-4"><span className="text-indigo-600 dark:text-indigo-400 font-medium truncate max-w-[180px] block" title={String(p.transaction_id || p.mpesa_message || 'N/A')}>{p.transaction_id || p.mpesa_message || 'N/A'}</span></td><td className="px-5 py-4 text-center">{p.receipt_url ? ( <div className="flex items-center justify-center space-x-2"><button onClick={() => handleViewReceipt(p.receipt_url!)} className="font-medium py-1 px-2.5 rounded-md text-sm" title="View"><Eye size={16} /></button><button onClick={() => handleDownloadReceipt(p)} disabled={currentIsDownloading} className={`font-medium py-1 px-2.5 rounded-md text-sm ${currentIsDownloading ? 'opacity-60' : ''}`} title="Download">{currentIsDownloading ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />}</button></div> ) : (<span className="text-xs italic text-slate-400">N/A</span>)}</td></tr> ); })) : ( <tr><td colSpan={8} className="text-center py-16 text-slate-500"><div className="flex flex-col items-center"><Search size={48} className="mb-4 text-slate-400" />No payments match your filters.</div></td></tr> )}</tbody></table></div>
            </div>
            ...
            {currentUserId && isMpesaModalOpen && <MpesaPayment isOpen={isMpesaModalOpen} onClose={handleClosePaymentModals} userId={currentUserId} />}
            ...
            {currentUserId && isManualEntryModalOpen && <ManualPaymentEntryModal isOpen={isManualEntryModalOpen} onClose={handleClosePaymentModals} />}
        </div>
    );
};
