...
import {
    ...
    CreditCard, Activity, Eye, Download, Edit3, UserX,
} from 'lucide-react';
...
import {
    ...
    selectIsAuthenticated,
     // Import this if you want to use it directly, otherwise derive from selectCurrentUser
...
const UserPaymentPortal: React.FC = () => {
    const loggedInUser = useSelector(selectCurrentUser); // Correctly gets the User object or null
    const isAuthenticated = useSelector(selectIsAuthenticated); // Correctly gets boolean based on token and user

    // const currentUserId = useSelector(selectCurrentUserId); // Alternative direct selector
    const currentUserId = loggedInUser?.user_id; // Deriving from loggedInUser
    ...
    useEffect(() => {
        // No authLoading check here as the simple slice doesn't have it
    ...
    const handleFilter = useCallback(() => {
        ...
        // ... (rest of the filter logic is fine) ...
    ...
    const resetFilters = () => {
        ...
        if (isAuthenticated && currentUserId) {
            ...
            toast.info("Filters have been reset for your payments.");
        }
    };
    ...
    // handleDownloadReceipt and handleViewReceipt (These remain the same)
    const handleDownloadReceipt = async (payment: PaymentDataTypes) => {
        ...
        toast.promise(
            (async () => {
                try { // Added try block for setIsDownloading
                    ...
                    return { fileName };
                } finally { // Ensure setIsDownloading(null) is always called
    ...
    // REMOVED the authLoading specific return block.
    // The logic now falls through to the !isAuthenticated check.
        ...
        return (
            <div className="w-full p-4 md:p-6 bg-gray-50 dark:bg-gray-900 min-h-screen font-sans flex justify-center items-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">Please log in to view your payment portal.</p>
                </div>
            </div>
        );
    }
    
    // Use full_name for the portal title
    const portalTitle = loggedInUser?.full_name ? `Your Payment Portal (${loggedInUser.full_name})` : `Your Personal Payment Portal`;
    ...
    return (
        <div className="w-full p-4 md:p-6 bg-gray-50 dark:bg-gray-900 min-h-screen font-sans">
                        ...
                        <div className="origin-top-right absolute right-0 mt-2 w-56 rounded-md shadow-xl bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-20">
                            <div className="py-1">
                                {paymentModes.map(({ mode, label, icon }) => (
                                    <button key={mode} onClick={() => handlePaymentModeSelect(mode)} className="flex items-center w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white">
                                        {icon} {label}
                                    </button>
                                ))}
                            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full leading-normal">
                        ...
                        <tbody className="text-gray-700 dark:text-gray-300 text-sm">
                            ...
                            {paymentsLoading && baseUserPayments.length === 0 ? ( // Only show loading if payments are actually loading and no base data yet
                                <tr><td colSpan={8} className="text-center py-10"><div className="flex justify-center items-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500"></div><span className="ml-3">Loading your payments...</span></div></td></tr>
                            ) : isError && baseUserPayments.length === 0 && !paymentsLoading ? (
                                <tr><td colSpan={8} className="text-center py-10 text-red-500 dark:text-red-400"><div className="flex flex-col items-center"><AlertTriangle size={40} className="mb-2" /> Error loading your payments.</div></td></tr>
                            ) : !paymentsLoading && baseUserPayments.length === 0 && isAuthenticated ? ( // Show "no records" if not loading and authenticated
                                <tr><td colSpan={8} className="text-center py-10 text-gray-500 dark:text-gray-400"><div className="flex flex-col items-center"><Search size={40} className="mb-2 text-gray-400 dark:text-gray-500" />You have no payment records yet.</div></td></tr>
                               ...
                               !paymentsLoading && baseUserPayments.length > 0 && filteredPayments.length === 0 && isAuthenticated && (
                                <tr><td colSpan={8} className="text-center py-10 text-gray-500 dark:text-gray-400"><div className="flex flex-col items-center"><Search size={40} className="mb-2 text-gray-400 dark:text-gray-500" />No payments found matching your current filters.</div></td></tr>
                               )
                            )}
                        </tbody>
                    </table>
            ...
            {/* Modals - Pass currentUserId if not null */}
            {currentUserId && isCashModalOpen && <CashPaymentModal isOpen={isCashModalOpen} onClose={handleClosePaymentModals} userId={currentUserId} />}
            ...
            {currentUserId && isStripeModalOpen && <StripePaymentModal isOpen={isStripeModalOpen} onClose={handleClosePaymentModals} userId={currentUserId} />}
            {currentUserId && isManualEntryModalOpen && <ManualPaymentEntryModal isOpen={isManualEntryModalOpen} onClose={handleClosePaymentModals} userId={currentUserId} />}
        </div>
    );
};
