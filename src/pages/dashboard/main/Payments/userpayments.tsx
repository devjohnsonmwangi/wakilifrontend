...
import {
    ListFilter, RotateCcw, Search, CalendarDays, ChevronDown,
...
const UserPaymentRecords: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (data?.success && Array.isArray(data.payments)) {
            ...
            if (isError && !paymentsLoading) toast.error("Error fetching your payment records.");
            else if (data && !data.success && !paymentsLoading) toast.warning("Could not fetch your payment records successfully.");
        }
    ...
    const handleFilter = useCallback(() => {
        ...
        if (filterDateFrom && filterDateFrom.length > 0) {
        ...
        if (filterDateTo && filterDateTo.length > 0) {
        ...
        return (
            <div className="w-full p-4 md:p-6 bg-slate-100 dark:bg-slate-900 min-h-screen font-sans flex justify-center items-center transition-colors duration-300">
                <div className="text-center p-8 bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-md w-full">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Please log in to view your payment records.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="w-full p-4 md:p-6 bg-slate-100 dark:bg-slate-900 min-h-screen font-sans transition-colors duration-300">
            ...
            <div className="mb-8 p-5 sm:p-6 bg-white dark:bg-slate-800 rounded-xl shadow-lg">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-5">
                    <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 flex items-center mb-3 sm:mb-0"> <ListFilter size={18} className="mr-2.5 text-indigo-500 dark:text-indigo-400" /> Filter Records </h3>
                    ...
                    <table className="min-w-full">
                        ...
                        <tbody className="text-slate-700 dark:text-slate-300 text-sm divide-y divide-slate-200 dark:divide-slate-700">
                            {paymentsLoading && baseUserPayments.length === 0 ? (
                                <tr><td colSpan={8} className="text-center py-12"><div className="flex justify-center items-center text-slate-500 dark:text-slate-400"><Loader2 size={24} className="animate-spin mr-3" />Loading payment records...</div></td></tr>
                            ) : isError && baseUserPayments.length === 0 && !paymentsLoading ? (
                                <tr><td colSpan={8} className="text-center py-12"><div className="flex flex-col items-center text-red-500 dark:text-red-400"><AlertTriangle size={40} className="mb-3" /> Error loading records. Please try again.</div></td></tr>
                            ) : !paymentsLoading && baseUserPayments.length === 0 && isAuthenticated ? (
                                <tr><td colSpan={8} className="text-center py-16 text-slate-500 dark:text-slate-400"><div className="flex flex-col items-center"><Search size={48} className="mb-4 text-slate-400 dark:text-slate-500" />You have no payment records.</div></td></tr>
                               ...
                               !paymentsLoading && baseUserPayments.length > 0 && filteredPayments.length === 0 && isAuthenticated && (
                                <tr><td colSpan={8} className="text-center py-16 text-slate-500 dark:text-slate-400"><div className="flex flex-col items-center"><Search size={48} className="mb-4 text-slate-400 dark:text-slate-500" />No records found matching your filters.</div></td></tr>
                               )
                            )}
                        </tbody>
                    </table>
