        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-50 p-4">
            <form onSubmit={handleSubmit} className="relative bg-slate-100 dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-4xl max-h-[95vh] flex flex-col">
                <header className="flex items-center justify-between p-4 border-b rounded-t dark:border-slate-700 flex-shrink-0">
                    ...
                    <button type="button" className="p-1.5 rounded-full text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700" onClick={handleClose} title="Close" aria-label="Close"><X className="h-5 w-5"  /></button>
                </header>
                ...
                <div className="p-4 md:p-6 space-y-4 overflow-y-auto">
                    ...
                    <div className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {(selectedGateway === 'mpesa' || selectedGateway === 'stripe' || selectedGateway === 'bank_transfer') && ( <div className="relative"><label htmlFor="manualTransactionId" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Transaction/Ref ID</label><Search className="absolute left-3 top-10 -translate-y-1/2 h-4 w-4 text-slate-400" aria-hidden="true"/><input type="text" id="manualTransactionId" value={transactionId} onChange={e => setTransactionId(e.target.value)} className={`${inputBaseClasses} pl-9`} placeholder={selectedGateway === 'mpesa' ? "e.g. RG830P81AA" : "Transaction ID"}/></div> )}
