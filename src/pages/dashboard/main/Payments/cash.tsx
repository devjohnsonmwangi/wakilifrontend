        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col">
                <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
                    ...
                    <button type="button" onClick={handleClose} className="p-2 rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700" title="Close" aria-label="Close"><X className="h-5 w-5"  /></button>
                </header>
                ...
                <div className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-6">
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                            <div ref={statusDropdownRef} className="relative"><label className="text-xs font-semibold mb-1 block flex items-center gap-1.5"><Info size={14}/>Status</label><button type="button" onClick={() => setIsStatusDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`}><span className={`truncate font-semibold ${!statusFilter ? 'text-slate-400 font-normal' : ''}`}>{statusFilter ? statusFilter.replace(/_/g, ' ') : 'All Statuses'}</span><ChevronDown size={20} /></button><AnimatePresence>{isStatusDropdownOpen && <motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button className="w-full text-left p-2.5 text-sm hover:bg-blue-50 dark:hover:bg-slate-700" onClick={() => { setStatusFilter(''); setIsStatusDropdownOpen(false); }}>All Statuses</button></div>{caseStatuses.map(s => <div key={s}><button className="w-full text-left p-2.5 text-sm hover:bg-blue-50 dark:hover:bg-slate-700" onClick={() => { setStatusFilter(s); setIsStatusDropdownOpen(false); }}>{s.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</button></div>)}</motion.div>}</AnimatePresence></div>
                            <div ref={typeDropdownRef} className="relative"><label className="text-xs font-semibold mb-1 block flex items-center gap-1.5"><Tag size={14}/>Case Type</label><button type="button" onClick={() => setIsTypeDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`}><span className={`truncate font-semibold ${!typeFilter ? 'text-slate-400 font-normal' : ''}`}>{typeFilter ? typeFilter.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'All Types'}</span><ChevronDown size={20} /></button><AnimatePresence>{isTypeDropdownOpen && <motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button className="w-full text-left p-2.5 text-sm hover:bg-blue-50 dark:hover:bg-slate-700" onClick={() => { setTypeFilter(''); setIsTypeDropdownOpen(false); }}>All Types</button></div>{caseTypes.map(t => <div key={t}><button className="w-full text-left p-2.5 text-sm hover:bg-blue-50 dark:hover:bg-slate-700" onClick={() => { setTypeFilter(t); setIsTypeDropdownOpen(false); }}>{t.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</button></div>)}</motion.div>}</AnimatePresence></div>
                            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div className="relative"><label className="text-xs font-semibold mb-1 block">Station</label><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" aria-hidden="true"/><input type="text" className={`${inputBaseClasses} py-2.5 pl-9 pr-8`} placeholder="Filter by Station" value={stationFilterInput} onChange={(e) => setStationFilterInput(e.target.value)} />{stationFilterInput && <button onClick={() => setStationFilterInput('')} title="Clear Station" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16}/></button>}</div>
                                <div className="relative"><label className="text-xs font-semibold mb-1 block">Case Number</label><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" aria-hidden="true"/><input type="text" className={`${inputBaseClasses} py-2.5 pl-9 pr-8`} placeholder="Filter by Case Number" value={caseNumberFilterInput} onChange={(e) => setCaseNumberFilterInput(e.target.value)} />{caseNumberFilterInput && <button onClick={() => setCaseNumberFilterInput('')} title="Clear Case Number" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16}/></button>}</div>
                                <div className="relative"><label className="text-xs font-semibold mb-1 block">General Search</label><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" aria-hidden="true"/><input type="text" className={`${inputBaseClasses} py-2.5 pl-9 pr-8`} placeholder="Parties, Track No..." value={searchTermInput} onChange={(e) => setSearchTermInput(e.target.value)} />{searchTermInput && <button onClick={() => setSearchTermInput('')} title="Clear Search" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16}/></button>}</div>
                            </div>
                        </div>
