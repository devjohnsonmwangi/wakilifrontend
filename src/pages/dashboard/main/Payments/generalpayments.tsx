            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/60 z-50">
                <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} onClick={(e) => e.stopPropagation()} className="absolute right-0 top-0 h-full w-full max-w-md bg-slate-50 dark:bg-slate-900 shadow-2xl flex flex-col max-h-screen">
                    <header className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
                        ...
                        <button onClick={onClose} title="Close" aria-label="Close" className="p-1 rounded-full text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700"><X size={20}  /></button>
                    </header>
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
                    ...
                    >
                        <button onClick={onClose} title="Close" aria-label="Close" className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors z-10"><X size={24}  /></button>
                        ...
                        <form onSubmit={handleFormSubmit} className="flex-grow contents">
                            <div className="p-6 sm:p-8 flex-grow overflow-y-auto styled-scrollbar">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                                    <div className="relative"><User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 dark:text-slate-500" aria-hidden="true"/><input type="text" placeholder="Customer Full Name*" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className={`${inputBaseClasses} py-3 pl-12 pr-10`} required />{customerName && (<button type="button" onClick={() => setCustomerName('')} title="Clear Name" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-full"><X size={16} /></button>)}</div>
                                    <div className="relative"><Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 dark:text-slate-500" aria-hidden="true"/><input type="email" placeholder="Customer Email (for receipt)" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)} className={`${inputBaseClasses} py-3 pl-12 pr-10`} />{customerEmail && (<button type="button" onClick={() => setCustomerEmail('')} title="Clear Email" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-full"><X size={16} /></button>)}</div>
                                    <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-base font-semibold text-slate-400 dark:text-slate-500" aria-hidden="true">KES</span><input type="number" placeholder="Amount*" value={amount} onChange={(e) => setAmount(e.target.value)} className={`${inputBaseClasses} py-3 pl-14 pr-10`} required min="1" />{amount && (<button type="button" onClick={() => setAmount('')} title="Clear Amount" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-full"><X size={16} /></button>)}</div>
                                    <div ref={methodDropdownRef} className="relative"><p className="text-sm font-semibold mb-2 text-slate-600 dark:text-slate-300">Payment Method*</p><button type="button" onClick={() => setIsMethodDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-3 px-4`}><span className={`truncate font-semibold ${!paymentMethod ? 'text-slate-400 dark:text-slate-500 font-normal' : ''}`}>{paymentMethod.length ? paymentMethod.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : 'Select a method...'}</span><ChevronDown size={20} className={`text-slate-400 transition-transform duration-200 ${isMethodDropdownOpen ? 'rotate-180' : ''}`} /></button><AnimatePresence>{isMethodDropdownOpen && (<motion.div initial={{opacity:0, y: -5}} animate={{opacity:1, y: 0}} exit={{opacity:0, y: -5}} className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg"><div><button type="button" className="w-full text-left px-4 py-3 text-sm font-semibold hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center" onClick={() => { setPaymentMethod('mpesa'); setIsMethodDropdownOpen(false); }}><Phone className="mr-2 h-4 w-4 text-green-500" /> M-Pesa</button></div><div><button type="button" className="w-full text-left px-4 py-3 text-sm font-semibold hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center" onClick={() => { setPaymentMethod('stripe'); setIsMethodDropdownOpen(false); }}><CreditCard className="mr-2 h-4 w-4 text-blue-500" /> Credit/Debit Card</button></div><div><button type="button" className="w-full text-left px-4 py-3 text-sm font-semibold hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center" onClick={() => { setPaymentMethod('cash'); setIsMethodDropdownOpen(false); }}><Wallet className="mr-2 h-4 w-4 text-yellow-500" /> Cash</button></div><div><button type="button" className="w-full text-left px-4 py-3 text-sm font-semibold hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center" onClick={() => { setPaymentMethod('bank_transfer'); setIsMethodDropdownOpen(false); }}><Landmark className="mr-2 h-4 w-4 text-purple-500" /> Bank Transfer</button></div></motion.div>)}</AnimatePresence></div>
                                    <AnimatePresence>{paymentMethod === 'mpesa' && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="relative overflow-hidden md:col-span-2"><Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 dark:text-slate-500"/><input type="tel" placeholder="Safaricom Phone Number* (e.g. 07...)" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value.replace(/\D/g, '').slice(0, 10))} className={`${inputBaseClasses} py-3 pl-12 pr-10`} required />{phoneNumber && (<button type="button" onClick={() => setPhoneNumber('')} title="Clear Phone" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-full"><X size={16} /></button>)}</motion.div>)}</AnimatePresence>
        ...
        <>
            ...
            <div className="mb-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 items-end">
                    <div className="lg:col-span-1 xl:col-span-1"><label className="block text-xs font-semibold mb-1 text-slate-500 dark:text-slate-400">Search Name/Email</label><div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" aria-hidden="true"/><input type="text" name="term" placeholder="Name or Email..." value={filters.term} onChange={handleFilterChange} className="block w-full text-sm text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg shadow-inner py-2.5 pl-10 pr-4" /></div></div>
                    <div className="lg:col-span-1 xl:col-span-1"><label className="block text-xs font-semibold mb-1 text-slate-500 dark:text-slate-400">Transaction ID</label><div className="relative"><Hash className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" aria-hidden="true"/><input type="text" name="transactionId" placeholder="Transaction ID..." value={filters.transactionId} onChange={handleFilterChange} className="block w-full text-sm text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg shadow-inner py-2.5 pl-10 pr-4" /></div></div>
                    <div className="xl:col-span-1"><label htmlFor="method-select" className="block text-xs font-semibold mb-1 text-slate-500 dark:text-slate-400">Method</label><select id="method-select" name="method" value={filters.method} onChange={handleFilterChange} className="block w-full text-sm text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg shadow-inner py-2.5 px-4" title="Method" aria-label="Method"><option value="">All Methods</option><option value="mpesa">M-Pesa</option><option value="stripe">Card</option><option value="cash">Cash</option><option value="bank_transfer">Bank Transfer</option></select></div>
                    <div className="lg:col-span-1 xl:col-span-1"><label className="block text-xs font-semibold mb-1 text-slate-500 dark:text-slate-400">Date Range</label><div className="grid grid-cols-2 gap-2"><input type="date" name="dateFrom" title="From Date" value={filters.dateFrom} onChange={handleFilterChange} className="block w-full text-sm text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg shadow-inner py-2.5 px-2" /><input type="date" name="dateTo" title="To Date" value={filters.dateTo} onChange={handleFilterChange} className="block w-full text-sm text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-lg shadow-inner py-2.5 px-2" min={filters.dateFrom}/></div></div>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700/50 styled-scrollbar">
                <table className="w-full text-sm">
                    <thead className="bg-blue-600 text-white">
                        <tr>
                            <th className="p-4"><input type="checkbox" title="Select" aria-label="Select" className="form-checkbox bg-blue-500 border-white" checked={isAllSelectedOnPage} onChange={handleSelectAll}/></th>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        {!isLoadingPayments && paginatedPayments.map((p) => (
                           <tr key={p.payment_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors duration-200">
                                <td className="p-4"><input type="checkbox" title="Select" aria-label="Select" className="form-checkbox" checked={selectedPayments.includes(p.payment_id)} onChange={(e) => handleSelectOne(p.payment_id, e.target.checked)}/></td>
                                <td className="p-4">
            ...
            <div>
                ...
                <div className="bg-white dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700/50">
                    <ul className="divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        )) : <li className="text-center text-slate-500 p-4">Not enough customer data to generate a leaderboard.</li>}
                    </ul>
                </div>
            </div>
        </div>
                ...
                <div className="max-w-screen-2xl mx-auto">
                    <header className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
                        ...
                        <div className='flex items-center gap-3'>
                            ...
                            <button onClick={toggleDarkMode} title="Toggle dark mode" aria-label="Toggle dark mode" className="p-3 rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300"><Sun className={`h-5 w-5 ${isDarkMode ? 'block' : 'hidden'}`}/><Moon className={`h-5 w-5 ${isDarkMode ? 'hidden' : 'block'}`}/></button>
