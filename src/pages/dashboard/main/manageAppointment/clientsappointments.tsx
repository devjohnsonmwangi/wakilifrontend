        ...
        <section className="p-4 md:p-6">
          <div className="mb-6 rounded-lg border border-sky-200 bg-sky-50/70 p-4 shadow-sm dark:border-sky-700 dark:bg-sky-700/30">
            ...
            <div className="grid grid-cols-1 items-end gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              <div className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" aria-hidden="true" /><input type="text" placeholder="Search by Staff Name..." value={searchStaffName} onChange={(e) => setSearchStaffName(e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{searchStaffName && <button onClick={() => setSearchStaffName('')} title="Clear Search" className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400 hover:text-slate-600"><X size={16} /></button>}</div>
              <div ref={locationDropdownRef} className="relative"><button onClick={() => setIsLocationDropdownOpen(p => !p)} className={`${inputBaseClasses} flex items-center justify-between py-2.5 px-4 text-left`} disabled={isLoadingBranchLocations}><span className={`truncate font-semibold ${!searchBranchId ? 'font-normal text-slate-400' : ''}`}>{isLoadingBranchLocations ? 'Loading...' : branchLocations?.find(b => b.branch_id === searchBranchId)?.name || 'All Locations'}</span><div className="flex items-center">{searchBranchId && <button onClick={(e) => { e.stopPropagation(); setSearchBranchId(''); }} title="Clear Location" className="mr-1 rounded-full p-1 text-slate-400 hover:text-slate-600"><X size={16} /></button>}<ChevronDown size={20} className={`text-slate-400 transition-transform ${isLocationDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isLocationDropdownOpen && <motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-lg border bg-white shadow-lg dark:border-slate-600 dark:bg-slate-900"><div><button onClick={() => { setSearchBranchId(''); setIsLocationDropdownOpen(false); }} className="w-full px-4 py-2.5 text-left text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50">All Locations</button></div>{branchLocations?.map(loc => <div key={loc.branch_id}><button onClick={() => { setSearchBranchId(loc.branch_id); setIsLocationDropdownOpen(false); }} className="w-full px-4 py-2.5 text-left text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50">{loc.name}</button></div>)}</motion.div>}</AnimatePresence></div>
              <div ref={statusFilterDropdownRef} className="relative"><button onClick={() => setIsStatusFilterDropdownOpen(p => !p)} className={`${inputBaseClasses} flex items-center justify-between py-2.5 px-4 text-left`}><span className={`truncate font-semibold ${!searchStatus ? 'font-normal text-slate-400' : ''}`}>{searchStatus ? getStatusDisplayName(searchStatus) : 'All Statuses'}</span><div className="flex items-center">{searchStatus && <button onClick={(e) => { e.stopPropagation(); setSearchStatus(''); }} title="Clear Status" className="mr-1 rounded-full p-1 text-slate-400 hover:text-slate-600"><X size={16} /></button>}<ChevronDown size={20} className={`text-slate-400 transition-transform ${isStatusFilterDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isStatusFilterDropdownOpen && <motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-lg border bg-white shadow-lg dark:border-slate-600 dark:bg-slate-900"><div><button onClick={() => { setSearchStatus(''); setIsStatusFilterDropdownOpen(false); }} className="w-full px-4 py-2.5 text-left text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50">All Statuses</button></div>{APPOINTMENT_STATUS_VALUES.map(status => <div key={status}><button onClick={() => { setSearchStatus(status); setIsStatusFilterDropdownOpen(false); }} className="w-full px-4 py-2.5 text-left text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50">{getStatusDisplayName(status)}</button></div>)}</motion.div>}</AnimatePresence></div>
              <div className="grid grid-cols-2 gap-2"><div className="relative"><input type="date" title="Date From" value={searchDateFrom} onChange={e => setSearchDateFrom(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} />{searchDateFrom && <button onClick={() => setSearchDateFrom('')} title="Clear Date" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400"><X size={16} /></button>}</div><div className="relative"><input type="date" title="Date To" value={searchDateTo} onChange={e => setSearchDateTo(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} min={searchDateFrom || undefined} />{searchDateTo && <button onClick={() => setSearchDateTo('')} title="Clear Date" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400"><X size={16} /></button>}</div></div>
            </div>
          </div>
