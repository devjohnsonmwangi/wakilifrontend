      ...
      <div className="max-w-full mx-auto bg-white dark:bg-slate-800 shadow-2xl rounded-xl overflow-hidden">
        ...
        <section className="p-4 md:p-6">
          ...
          <div className="mb-6 p-4 bg-sky-50/70 dark:bg-sky-700/30 rounded-lg shadow-sm border border-sky-200 dark:border-sky-700">
            ...
            (<div ref={branchDropdownRef} className="relative md:w-1/2 lg:w-1/3"><button type="button" onClick={() => setIsBranchDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4 ${!selectedBranchId ? 'text-slate-400' : ''}`}>{selectedBranchName || '-- Choose a Branch --'}<div className="flex items-center">{selectedBranchId && (<button type="button" onClick={(e) => { e.stopPropagation(); clearSelectedBranch();}} title="Clear Selection" className="p-1 mr-1 text-slate-400 hover:text-slate-600 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 transition-transform ${isBranchDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isBranchDropdownOpen && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-30 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">{allBranches?.map(branch => (<div key={branch.branch_id}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50" onClick={() => handleSelectBranch(branch)}>{branch.name}</button></div>))}</motion.div>)}</AnimatePresence></div>)}
          </div>
          ...
          {selectedBranchId && (
            <div className="mb-6 p-4 bg-slate-50/70 dark:bg-slate-700/30 rounded-lg shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-end">
                <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" aria-hidden="true"/><input type="text" placeholder="Client Name..." value={searchClientName} onChange={(e) => setSearchClientName(e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{searchClientName && (<button onClick={() => setSearchClientName('')} title="Clear Search" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16} /></button>)}</div>
                <div className="relative"><input type="number" placeholder="Client ID..." value={searchClientId} onChange={(e) => setSearchClientId(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-10`} />{searchClientId && (<button onClick={() => setSearchClientId('')} title="Clear ID" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16} /></button>)}</div>
                <div ref={statusFilterDropdownRef} className="relative"><button type="button" onClick={() => setIsStatusFilterDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4 ${!searchStatus ? 'text-slate-400' : ''}`}>{searchStatus ? getStatusDisplayName(searchStatus) : 'All Statuses'}<div className="flex items-center">{searchStatus && (<button type="button" onClick={(e) => { e.stopPropagation(); setSearchStatus(''); }} title="Clear Filter" className="p-1 mr-1 text-slate-400 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 transition-transform ${isStatusFilterDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isStatusFilterDropdownOpen && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border rounded-lg shadow-lg max-h-60 overflow-y-auto"><div key="all"><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50" onClick={() => setSearchStatus('')}>All Statuses</button></div>{APPOINTMENT_STATUS_VALUES.map(status => (<div key={status}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50" onClick={() => setSearchStatus(status)}>{getStatusDisplayName(status)}</button></div>))}</motion.div>)}</AnimatePresence></div>
                <div className="grid grid-cols-2 gap-2"><div className="relative"><input type="date" title="Date From" value={searchDateFrom} onChange={e => setSearchDateFrom(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} />{searchDateFrom && (<button onClick={() => setSearchDateFrom('')} title="Clear Date" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16} /></button>)}</div><div className="relative"><input type="date" title="Date To" value={searchDateTo} onChange={e => setSearchDateTo(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} min={searchDateFrom || undefined} />{searchDateTo && (<button onClick={() => setSearchDateTo('')} title="Clear Date" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 rounded-full"><X size={16} /></button>)}</div></div>
              </div>
            </div>
          )}
