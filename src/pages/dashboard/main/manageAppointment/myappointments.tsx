...
import {
  ...
  User, // Fallback icon
  LogIn,
  ImageOff, // Icon for broken image
} from 'lucide-react';

// ... (ApiError, ApiMessageResponse, isApiMessageResponse, NO_APPOINTMENTS_FOUND_MESSAGE, getErrorMessage interfaces/functions remain the same) ...
interface ApiError {
...
const UserAppointments: React.FC = () => {
  ...
  // Assuming your currentUser object has a field for the profile picture URL
  const profilePictureUrl = currentUser?.profile_picture || null; // ADJUST 'profile_picture_url' if needed
  ...
  // Reset image error state if the profile picture URL changes
  useEffect(() => {
    setImgError(false);
  }, [profilePictureUrl]);
  ...
  const safeAppointmentsData = useMemo(() => {
    ...
    return [];
  }, [appointmentsData, isBackendNoAppointments, displayableError, userId]);
  ...
  const isEffectivelyInitialEmpty = !isActuallyLoading && !displayableError && !isFilteringActive && (isBackendNoAppointments || (Array.isArray(appointmentsData) && appointmentsData.length === 0 && !isFetchAppointmentsError) || (isApiMessageResponse(appointmentsData) && appointmentsData.message === NO_APPOINTMENTS_FOUND_MESSAGE));
  const showNoAppointmentsMessage = !isActuallyLoading && !displayableError && filteredAppointments.length === 0;


  // Fallback component for profile picture
  const ProfilePictureFallback: React.FC<{ name?: string | null, size?: number }> = ({ name, size = 8 }) => {
    ...
    const classSize = `h-${size} w-${size}`; // e.g. h-8 w-8 or h-10 w-10

    if (imgError && profilePictureUrl) { // If there was a URL but it failed
        return (
              ...
              <div className="mr-3 flex-shrink-0">
                {profilePictureUrl && !imgError ? (
                  <img
                    ...
                    className="h-10 w-10 rounded-full object-cover border-2 border-sky-200 dark:border-sky-700" // Adjust size (h-10 w-10) as needed
                    onError={() => setImgError(true)} 
                  />
                ) : (
                  <ProfilePictureFallback name={userFullName} size={10} /> // Pass size to match img tag
                )}
              </div>
