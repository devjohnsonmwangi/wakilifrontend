        ...
        <section className="p-4 md:p-6">
          <div className="mb-6 p-4 bg-sky-50/70 dark:bg-sky-700/30 rounded-lg shadow-sm border border-sky-200 dark:border-sky-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 pointer-events-none" aria-hidden="true"/><input type="text" placeholder="Client Name..." value={searchClient} onChange={(e) => setSearchClient(e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{searchClient && (<button type="button" onClick={() => setSearchClient('')} title="Clear Client" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full"><X size={16} /></button>)}</div>
              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 pointer-events-none" aria-hidden="true"/><input type="text" placeholder="Location Name..." value={searchLocation} onChange={(e) => setSearchLocation(e.target.value)} className={`${inputBaseClasses} py-2.5 pl-10 pr-10`} />{searchLocation && (<button type="button" onClick={() => setSearchLocation('')} title="Clear Location" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full"><X size={16} /></button>)}</div>
              {/* Status Dropdown */}
              <div ref={statusDropdownRef} className="relative"><button type="button" onClick={() => setIsStatusDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`}><span className={`truncate font-semibold ${!searchStatus ? 'text-slate-400 dark:text-slate-500 font-normal' : ''}`}>{searchStatus ? getStatusDisplayName(searchStatus) : 'All Statuses'}</span><div className="flex items-center">{searchStatus && (<button type="button" onClick={(e) => { e.stopPropagation(); setSearchStatus(''); }} title="Clear Status" className="p-1 mr-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 transition-transform duration-200 ${isStatusDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isStatusDropdownOpen && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50" onClick={() => { setSearchStatus(''); setIsStatusDropdownOpen(false); }}>All Statuses</button></div>{APPOINTMENT_STATUS_VALUES.map(status => (<div key={status}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { setSearchStatus(status); setIsStatusDropdownOpen(false); }}>{getStatusDisplayName(status)}</button></div>))} </motion.div>)}</AnimatePresence></div>
              {/* Branch Dropdown */}
              <div ref={branchDropdownRef} className="relative"><button type="button" onClick={() => setIsBranchDropdownOpen(p => !p)} className={`${inputBaseClasses} text-left flex justify-between items-center py-2.5 px-4`} disabled={isLoadingBranchLocations}><span className={`truncate font-semibold ${!searchBranchId ? 'text-slate-400 dark:text-slate-500 font-normal' : ''}`}>{isLoadingBranchLocations ? 'Loading...' : (branchLocations?.find(b => b.branch_id.toString() === searchBranchId.toString())?.name || 'All Branches')}</span><div className="flex items-center">{searchBranchId && (<button type="button" onClick={(e) => { e.stopPropagation(); setSearchBranchId(''); }} title="Clear Branch" className="p-1 mr-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full"><X size={16}/></button>)}<ChevronDown size={20} className={`text-slate-400 transition-transform duration-200 ${isBranchDropdownOpen ? 'rotate-180' : ''}`} /></div></button><AnimatePresence>{isBranchDropdownOpen && !isLoadingBranchLocations && (<motion.div variants={dropdownListVariants} initial="hidden" animate="visible" exit="exit" className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg max-h-60 overflow-y-auto"><div><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold hover:bg-blue-50 dark:hover:bg-slate-700/50" onClick={() => { setSearchBranchId(''); setIsBranchDropdownOpen(false); }}>All Branches</button></div>{branchLocations?.map(loc => (<div key={loc.branch_id}><button type="button" className="w-full text-left px-4 py-2.5 text-sm font-semibold" onClick={() => { setSearchBranchId(loc.branch_id); setIsBranchDropdownOpen(false); }}>{loc.name}</button></div>))} </motion.div>)}</AnimatePresence></div>
              {/* Other filters */}
              <div className="relative"><input type="number" placeholder="Client ID..." value={searchClientId} onChange={(e) => setSearchClientId(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-10`} />{searchClientId && (<button type="button" onClick={() => setSearchClientId('')} title="Clear Client ID" className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full"><X size={16} /></button>)}</div><div className="grid grid-cols-2 gap-2 col-span-1 sm:col-span-2 lg:col-span-1"><div className="relative"><input type="date" title="Date From" value={searchDateFrom} onChange={e => setSearchDateFrom(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} />{searchDateFrom && <button type="button" onClick={() => setSearchDateFrom('')} title="Clear Date From" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full focus:outline-none"><X size={16} /></button>}</div><div className="relative"><input type="date" title="Date To" value={searchDateTo} onChange={e => setSearchDateTo(e.target.value)} className={`${inputBaseClasses} py-2.5 px-4 pr-8`} min={searchDateFrom || undefined} />{searchDateTo && <button type="button" onClick={() => setSearchDateTo('')} title="Clear Date To" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-full focus:outline-none"><X size={16} /></button>}</div></div>
            </div>
          </div>
